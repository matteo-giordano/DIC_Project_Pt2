...
function initGeneratorConfig() {
    ...
    // Setup reset button
    document.getElementById('reset-generator-config').addEventListener('click', resetGeneratorConfig);
...
function setupArrayControls() {
    // Remove any existing event listeners from array-add buttons
    document.querySelectorAll('.array-add').forEach(btn => {
    ...
    document.querySelectorAll('.array-add').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // For width and height inputs, enforce odd numbers and set default value
            if (input.name.includes('width[]') || input.name.includes('height[]')) {
                input.min = '9';
                input.max = '51';
                input.step = '2';
                
                // Get all existing values in the container
                const existingValues = Array.from(container.querySelectorAll('.array-item-value'))
                    .map(input => parseInt(input.value))
                    .filter(value => !isNaN(value));
                
                // Find the next odd number that's not already used
                let nextValue = 21; // Default value
                if (existingValues.length > 0) {
                    const maxValue = Math.max(...existingValues);
                    nextValue = Math.min(51, maxValue + 2);
                }
                
                input.value = nextValue;
                
                // Add input validation to ensure odd numbers
                input.addEventListener('input', function() {
                    let value = parseInt(this.value);
                    if (isNaN(value)) {
                        value = 21;
                    }
                    // Ensure value is odd and within range
                    value = Math.max(9, Math.min(51, value));
                    if (value % 2 === 0) {
                        value = value + 1;
                    }
                    this.value = value;
...
function saveGeneratorConfig() {
    ...
    document.querySelectorAll('.config-section').forEach(section => {
        const generatorType = section.getAttribute('data-generator');
        if (!generatorType) {
            console.warn('Section missing data-generator attribute:', section);
            return;
        }
        
        // Check if the generator is enabled
        const toggleInput = section.querySelector('.generator-toggle');
        const isEnabled = toggleInput && toggleInput.checked;
        ...
        // Get count_per_config value - use more reliable selector
        const countInput = section.querySelector(`input[name="${generatorType}-count_per_config"]`);
        if (countInput) {
            config.generators[generatorType]['count_per_config'] = parseInt(countInput.value) || 10;
        } else {
            console.warn(`Count input not found for ${generatorType}, using default value of 10`);
            config.generators[generatorType]['count_per_config'] = 10;
        }
        
        // Get array parameters (width, height, and generator-specific ones)
        section.querySelectorAll('.array-container').forEach(container => {
            // Extract parameter name from data attribute
            const fullParamName = container.getAttribute('data-param-name');
            if (!fullParamName) return;
            
            // Remove generator type prefix (e.g., "prim-width" -> "width")
            const paramName = fullParamName.replace(`${generatorType}-`, '');
            const values = [];
            
            // Get parameter values from input fields
            container.querySelectorAll('.array-item-value').forEach(input => {
                // Handle numeric values properly
                if (container.getAttribute('data-param-type') === 'number') {
                    const step = container.getAttribute('data-param-step') || '1';
                    // Check if step contains a decimal point
                    if (step.includes('.')) {
            ...
            // Add to config if we have values
            if (values.length > 0) {
    ...
    // Get global settings
    const ensureConnected = document.getElementById('ensure-connected-global')?.checked ?? true;
    const ensureUnique = document.getElementById('ensure-unique-global')?.checked ?? true;
...
function generateFullDataset(progressBar, progressStatus) {
    ...
    // Get ensure_connected and ensure_unique values (with fallbacks)
    const ensureConnected = document.getElementById('ensure-connected-global')?.checked ?? true;
    const ensureUnique = document.getElementById('ensure-unique-global')?.checked ?? true;
    
    console.log(`Starting dataset generation (ensure_connected=${ensureConnected}, ensure_unique=${ensureUnique})`);
    ...
    .then(data => {
        console.log('Dataset generation response:', data);
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                console.error('Error redirecting to dataset:', err);
                                alert('Dataset was generated successfully, but there was an error loading it. Please go to the home page and load the dataset manually.');
...
function convertToYAML(obj) {
    try {
        ...
        if (!obj || typeof obj !== 'object') {
            console.error("Invalid object passed to convertToYAML", obj);
            return "name: 'default_dataset'\n\ngenerators:\n  prim:\n    count_per_config: 10\n    width: [21]\n    height: [21]";
        }
        
        // Start with dataset name
        let yaml = `name: "${obj.name || 'default_dataset'}"\n\ngenerators:\n`;
        
        // Safety check for generators
        if (!obj.generators || typeof obj.generators !== 'object' || Object.keys(obj.generators).length === 0) {
            console.warn("No generators found in object", obj);
            return yaml + "  prim:\n    count_per_config: 10\n    width: [21]\n    height: [21]";
        }
        ...
        for (const [genType, genConfig] of Object.entries(obj.generators)) {
            ...
            // Safety check for generator config
            if (!genConfig || typeof genConfig !== 'object') {
                console.warn(`Invalid config for generator ${genType}`, genConfig);
                yaml += `    count_per_config: 10\n    width: [21]\n    height: [21]\n`;
                continue;
            }
        ...
        console.error("Error in convertToYAML:", error);
        return "name: 'default_dataset'\n\ngenerators:\n  prim:\n    count_per_config: 10\n    width: [21]\n    height: [21]";
    }
}
...
function resetGeneratorConfig() {
    ...
    // Call the load_config endpoint to get the default configuration
    fetch('/load_config')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                // Reload the page to apply the default configuration
                window.location.reload();
            } else {
                alert('Error loading default configuration: ' + data.error);
            }
        })
        .catch(error => {
            alert('Error: ' + error.message);
