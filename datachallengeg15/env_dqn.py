...
class Environment:
    def __init__(self, array: np.ndarray, step_size=0.4, reward_config=None):
        self.maze = Maze(array=array, step_size=step_size)
        self.start_pos = self.maze.agent_pos
        self.reward_cfg = reward_config or {"progress": 2.0, "step": -0.01, "goal": 30.0}
    ...
    # def step(self, action: int): #PPO
    #     self.maze.step(action)
    #     return self._get_observation(), self.is_done()

    def step(self, action: int):
        prev_pos = self.maze.agent_pos.copy()        # store previous position
        prev_dist = np.linalg.norm(prev_pos - self.maze.goal_pos)
