...
class ActorNetwork(nn.Module):
    def __init__(self, input_size=15, hidden_size=128, output_size=8):
...
class PPO:
    def __init__(self, state_dim=13, action_dim=8, lr_actor=1e-3, lr_critic=3e-3, 
...
def train_ppo_on_maze(episodes=2000, max_steps_per_episode=1000, update_frequency=10, seed=42):
    ...
    for episode in tqdm(range(episodes)):
        ...
        episode_reward = 0
        episode_length = 0
        prev_distance = np.linalg.norm(env.maze.agent_pos - env.maze.goal_pos)
        ...
        for step in range(max_steps_per_episode):
            ...
            # Calculate reward with progress tracking
            current_distance = np.linalg.norm(env.maze.agent_pos - env.maze.goal_pos)
            reward = calculate_reward(env, done, prev_distance)
...
def calculate_reward(env, done, prev_distance):
    """
    ...
    if done:
        ...
        # Small negative reward for being far from goal (encourages goal-seeking)
        reward -= normalized_distance * 0.1
        
        # Progress reward: positive if getting closer, negative if getting farther
        progress = prev_distance - current_distance
        reward += progress * 2.0  # Amplify progress signal
        ...
        if current_distance < 2.0:
            reward += (2.0 - current_distance) * 0.5
    
    return reward


def test_trained_agent(model_path="ppo_maze_model.pth", episodes=5, seed=42):
    """
...
if __name__ == "__main__":
    ...
    # Train the agent
    agent, rewards, lengths = train_ppo_on_maze(episodes=110, max_steps_per_episode=2000, update_frequency=10, seed=SEED)
